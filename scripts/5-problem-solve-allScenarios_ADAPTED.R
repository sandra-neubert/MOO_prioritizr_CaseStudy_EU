...
)
{
  ...
  features_weighted <- targs |>
    mutate(weight = ifelse(feature == "999999", (nrow(targs)-1)*2*carbon_weight, weight)) |> #TODO: set a carbon weight?
...
#  }
  ...
  if(restoration_scenario == "HN"){ #TODO: chose high nature
    p_solve <- p_solve |>
  ...
  if(country_constraints == "UNEVEN"){ #TODO: chose an option
    p_solve <- p_solve
  }
  
  # wetland restoration
  
  if(wetlands == "TRUE"){ #TODO: choose whether to include
  ...
  # if(restoration_only == FALSE){
    ...
    if(future == "ref"){
      ...
      infeas_nuts2 <- read_csv("data/feasibility-tests/nuts2_feasible_ref_adj.csv") |> #TODO: why are these excluded?
        mutate(adj = ifelse(TF =="FALSE", 1,0))
      
      if(production_constraints %in% c("NUTS2adj", "NUTS2")){ #TODO: chose an option
...
# all the constraints
# IC_mismatch <- read_fst("data/plotting/plotting_data_ic.fst") |> group_by(id) |> #TODO: what is this for
